...
export async function createSchedule({ technician_id, scheduled_by, scheduled_at, week_ref, items_count, notes }) {
  const { data, error } = await db()
    .from('inventory_schedules')
    .insert({ technician_id, scheduled_by, scheduled_at, week_ref, items_count: items_count || 10, notes })
    .select()
    .single();
  if (error) throw error;
  return data;
