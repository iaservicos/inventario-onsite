...
export async function getTechnicianItems(technicianName) {
  // Usamos backticks (`) para proteger nomes de colunas com espaços ou caracteres especiais
  const sql = `
    SELECT 
    ...
    WHERE tecnico_nome = '${technicianName}' 
      ...
      AND (data_montagem_lote_dev IS NULL OR data_montagem_lote_dev = '' OR data_montagem_lote_dev = ' ')
      AND (data_envio_dev IS NULL OR data_envio_dev = '' OR data_envio_dev = ' ')
      AND (mont_lote IS NULL OR mont_lote = '' OR mont_lote = ' ')
