/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL ESTABILIZADA V5
 */
...
/**
 * BUSCA EM MASSA: Filtros originais estabilizados
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURAÇÃO TOTAL V5:
    // 1. Filtros originais de data/status.
    // 2. Filtro doc_faturamento: Apenas 'NÃO DEFINIDO' ou vazio para remover RMDFs faturados.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro RMDF: Peça nova legítima não tem faturamento definido
        AND (doc_faturamento IS NULL OR doc_faturamento = '' OR doc_faturamento = 'NÃO DEFINIDO')
    `;
