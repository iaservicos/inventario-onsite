...
/**
 * Busca o portfólio de peças com os nomes de colunas corretos do Databricks
 */
export async function getTechnicianItems(technicianName) {
  // Query atualizada com:
  // - cod_peca_enviada (Código)
  // - descr_peca_enviada (Descrição)
  // - qtd_peca_enviada (Quantidade)
  const sql = `
    SELECT 
    ...
    WHERE tecnico_nome = '${technicianName}' 
      ...
      AND (data_montagem_lote_dev IS NULL OR data_montagem_lote_dev = '')
      AND (data_envio_dev IS NULL OR data_envio_dev = '')
      AND (mont_lote IS NULL OR mont_lote = '')
    GROUP BY cod_peca_enviada, descr_peca_enviada, unidade
    ORDER BY descr_peca_enviada ASC
  `;
