/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL BLINDADA V3
 */
...
/**
 * BUSCA EM MASSA: Filtros Blindados Combinados
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // QUERY FINAL BLINDADA V3:
    // 1. Filtros originais de data/status.
    // 2. Filtro de chamado_pedido <> '0' (limpa RMDF do Felipe e a 36a do Marcio).
    // 3. Filtro de chamado_aplicado <> 'NÃO DEFINIDO' (limpa Amaury).
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro para RMDF e Backups (Felipe e Marcio): Deve ter chamado real
        AND (chamado_pedido IS NOT NULL AND chamado_pedido <> '0' AND chamado_pedido <> '')
        -- Filtro para Amaury: Chamado aplicado deve ser definido
        AND (chamado_aplicado IS NOT NULL AND UPPER(TRIM(chamado_aplicado)) <> 'NÃO DEFINIDO')
    `;
