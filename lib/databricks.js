/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL BLINDADA
 */
...
/**
 * BUSCA EM MASSA: Filtros combinados para Felipe (RMDF) e Amaury (Falso Positivo)
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // QUERY FINAL BLINDADA:
    // 1. Filtros originais de data/status.
    // 2. Filtro de doc_faturamento (limpa Felipe/RMDF).
    // 3. Filtro de chamado_aplicado (limpa Amaury/Falso Positivo).
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Regra Felipe (RMDF): doc_faturamento deve ser limpo
        AND (doc_faturamento IS NULL OR doc_faturamento = '' OR UPPER(TRIM(doc_faturamento)) = 'NÃO DEFINIDO')
        -- Regra Amaury: chamado_aplicado não pode ser 'NÃO DEFINIDO' ou '0'
        AND (chamado_aplicado IS NOT NULL 
             AND UPPER(TRIM(chamado_aplicado)) <> 'NÃO DEFINIDO' 
             AND UPPER(TRIM(chamado_aplicado)) <> '0'
             AND UPPER(TRIM(chamado_aplicado)) <> ''
            )
    `;
