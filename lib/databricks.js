/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO MESTRA ESTABILIZADA
 */
...
/**
 * BUSCA EM MASSA: Filtros originais + Filtro de Reserva (num_reserva)
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // SOLUÇÃO MESTRA:
    // 1. Filtros originais exatos ( Arnildo 105 peças ).
    // 2. Filtro num_reserva: Peças novas legítimas têm num_reserva vazio ou nulo.
    //    Se tiver número (como a 36a do Marcio), é RMDF/Reserva e não deve aparecer.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro de Reserva: Peça nova real NÃO tem número de reserva (é NULL ou vazio)
        AND (num_reserva IS NULL OR TRIM(CAST(num_reserva AS STRING)) = '' OR num_reserva = 'NÃO DEFINIDO')
    `;
