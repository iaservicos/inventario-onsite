/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL ESTABILIZADA V4
 */
...
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURAÇÃO TOTAL V4:
    // Voltamos exatamente para o seu SQL original.
    // O Marcio tem 507 peças com chamado_pedido = '0', então bloquear o '0' é o erro.
    // A única regra extra segura é tecnico_tipo <> 'LABORATORIO'.
    const sql = `
      SELECT
