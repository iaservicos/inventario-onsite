/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO BLINDADA 3.0
 */
...
/**
 * BUSCA EM MASSA: Com filtros originais + limpeza de RMDF/Backup/Estoque Técnico
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // FILTROS ORIGINAIS + LIMPEZA CIRÚRGICA:
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro para remover RMDF e registros sem chamado real
        AND chamado_pedido IS NOT NULL 
        AND TRIM(CAST(chamado_pedido AS STRING)) <> '0'
        AND TRIM(CAST(chamado_pedido AS STRING)) <> ''
        -- Filtro para remover Backups/Estoque de Técnico (TEC:XXXX)
        AND (chamado_aplicado IS NULL OR (
          UPPER(TRIM(chamado_aplicado)) <> 'NÃO DEFINIDO' 
          AND UPPER(TRIM(chamado_aplicado)) <> 'DEVNEW'
          AND UPPER(TRIM(chamado_aplicado)) NOT LIKE 'TEC:%'
        ))
    `;
