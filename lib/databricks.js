/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO CIRÚRGICA
 */
...
/**
 * BUSCA EM MASSA: Com filtros originais + limpeza cirúrgica de chamados inválidos
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURADO + FILTRO CIRÚRGICO:
    // Adicionamos a exclusão de 'NÃO DEFINIDO' e 'DEVNEW' no chamado_aplicado.
    // Isso remove a peça do Amaury sem afetar as 105 peças boas do Arnildo.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro Cirúrgico para remover registros "fantasmas" como o do Amaury
        AND (chamado_aplicado IS NULL OR (UPPER(TRIM(chamado_aplicado)) <> 'NÃO DEFINIDO' AND UPPER(TRIM(chamado_aplicado)) <> 'DEVNEW'))
    `;
