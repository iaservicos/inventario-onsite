/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO MESTRA ESTABILIZADA V6
 */
...
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURAÇÃO TOTAL V6:
    // 1. Filtros originais exatos de data/status (Arnildo 105 / Marcio 35).
    // 2. Filtro tecnico_tipo: Remove peças de laboratório que não são de campo.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro de Laboratório: Peça nova legítima é de técnico EXTERNO
        AND UPPER(TRIM(tecnico_tipo)) <> 'LABORATORIO'
    `;
