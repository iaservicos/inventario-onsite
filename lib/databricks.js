/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO INTELIGENTE
 */
...
/**
 * BUSCA EM MASSA: Com filtros originais + Regra Inteligente de Chamado Aplicado
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // FILTROS ORIGINAIS + REGRA INTELIGENTE:
    // 1. Mantemos os filtros originais de NULL/VAZIO que o usuário confia.
    // 2. Filtramos apenas o que é comprovadamente "lixo": NÃO DEFINIDO e DEVNEW.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtramos apenas o que é lixo confirmado, permitindo TEC: (serviços do Arnildo)
