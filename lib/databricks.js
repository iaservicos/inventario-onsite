/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO MESTRA ESTABILIZADA V7
 */
...
/**
 * BUSCA EM MASSA: Utiliza a nova View customizada com filtros embutidos
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    const sql = `
      ...
      WHERE technician_name_key IN (${placeholders})
    `;
...
const DATABRICKS_USED_TABLE =
  process.env.DATABRICKS_USED_TABLE ||
  'datalake_prod.indicadores_servicos.vw_inventario_onsite_usado_rmdf';

export async function getAllTechniciansUsedItems(technicianNames) {
  if (!technicianNames || technicianNames.length === 0) return [];
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    try {
      ...
      console.error(`Erro no lote usadas ${i}:`, err.message);
    }
  }
...
const DATABRICKS_DEVOLUCOES_TABLE =
  process.env.DATABRICKS_DEVOLUCOES_TABLE ||
  'datalake_prod.indicadores_servicos.vw_dev_tecnico_pendente';

export async function getAllTechniciansPendingReturns(technicianNames) {
  if (!technicianNames || technicianNames.length === 0) return [];
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    const sql = `
      SELECT
        ...
        data_montagem_lote,
        data_envio_lote,
        data_recebimento_atp,
        data_recusado_atp,
        data_recebimento_tecnico,
        dias_aguardando,
        dt_materializacao
      FROM ${DATABRICKS_DEVOLUCOES_TABLE}
      WHERE technician_name_key IN (${placeholders})
    `;
    ...
    try {
      ...
      console.error(`Erro no lote devolucoes ${i}:`, err.message);
    }
  }
...
export async function verifyTechnicianInDatabricks(technicianName) {
  ...
  const sql = `SELECT technician_name_key FROM ${DATABRICKS_TABLE} WHERE technician_name_key = UPPER(TRIM(:technicianName)) LIMIT 1`;
  const result = await runDatabricksQuery(sql, [{ name: 'technicianName', value: cleanName }]);
  return {
    found: result.length > 0,
    databricks_name: result.length > 0 ? result[0].technician_name_key : null,
