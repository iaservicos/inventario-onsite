/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FIEL ORIGINAL
 */
...
export async function runDatabricksQuery(sql, parameters = [], maxWaitMs = 300000) {
  ...
  // Busca páginas adicionais se houver (PAGINAÇÃO INFINITA)
  while (nextToken) {
...
/**
 * BUSCA EM MASSA: Busca peças de múltiplos técnicos (FILTROS EXATOS DO USUÁRIO)
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURADO EXATAMENTE COMO O USUÁRIO TINHA NO ARQUIVO ORIGINAL
    const sql = `
      SELECT
