/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL ESTABILIZADA V8
 */
...
/**
 * BUSCA EM MASSA: Filtros originais + Inteligência Anti-Duplicidade
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // SOLUÇÃO DEFINITIVA V8:
    // O Datalake tem múltiplas linhas para a mesma remessa (uma NOVO e outra USADO/ENCERRADO).
    // Usamos ROW_NUMBER para pegar apenas a linha mais relevante de cada remessa.
    const sql = `
      WITH RankedItems AS (
        SELECT
          ...
          mont_lote,
          ROW_NUMBER() OVER (
            PARTITION BY UPPER(TRIM(tecnico_nome)), num_remessa, cod_peca_enviada 
            ORDER BY 
              CASE WHEN status_consumo = 'USADO' THEN 1 ELSE 2 END ASC,
              CASE WHEN encerramento IS NOT NULL AND encerramento <> '1970-01-01T00:00:00.000Z' THEN 1 ELSE 2 END ASC,
              CASE WHEN mont_lote IS NOT NULL THEN 1 ELSE 2 END ASC
          ) as rank
      ...
      WHERE rank = 1
        ...
        AND (encerramento IS NULL OR TRIM(CAST(encerramento AS STRING)) = '' OR encerramento = '1970-01-01T00:00:00.000Z')
    `;
