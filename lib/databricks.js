...
/**
 * Busca o portfólio de peças do técnico (INCLUINDO REMESSA)
 */
export async function getTechnicianItems(technicianName) {
  // Limpa o nome para evitar problemas de espaços
...
/**
 * Verifica se o técnico existe no Databricks (Lógica Robusta)
 */
export async function verifyTechnicianInDatabricks(technicianName) {
  ...
  // Retorna um objeto com a informação se encontrou e o nome exato no Databricks
  return {
    found: result.length > 0,
    databricks_name: result.length > 0 ? result[0].tecnico_nome : null,
    total_items: result.length > 0 ? 1 : 0 // Simplificado para verificação rápida
  };
}
