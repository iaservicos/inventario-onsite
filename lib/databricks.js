/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL DE PRECISÃO
 */
...
/**
 * Executa query com suporte a paginação
 */
...
/**
 * BUSCA EM MASSA: Com filtros originais + Regra de Backup de Técnico
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // FILTRO DE PRECISÃO:
    // 1. Mantemos seus filtros originais de NULL/VAZIO.
    // 2. Removemos 'NÃO DEFINIDO' e 'DEVNEW' (Caso Amaury).
    // 3. Removemos peças onde o chamado_aplicado é o próprio código do técnico (Caso Felipe/RMDF).
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Filtro para remover lixo confirmado
        AND (chamado_aplicado IS NULL 
             OR (UPPER(TRIM(chamado_aplicado)) <> 'NÃO DEFINIDO' 
                 AND UPPER(TRIM(chamado_aplicado)) <> 'DEVNEW'
                 AND UPPER(TRIM(chamado_aplicado)) <> '0'
                )
            )
    `;
    ...
    try {
      ...
      // FILTRO DE BACKUP EM MEMÓRIA (CIRÚRGICO):
      // Removemos peças onde o chamado_aplicado contém o código que está no campo 'tecnico_id' ou similar.
      // Como o Datalake envia o chamado_aplicado como 'TEC:438438', e o Felipe é o 438438, nós filtramos.
      const filteredResult = result.filter(item => {
        const chamado = (item.chamado_aplicado || '').toUpperCase();
        // Se o chamado for 'TEC:CÓDIGO', e esse código for o que você identifica como backup, ele some.
        // No caso do Felipe, o chamado_aplicado é 'TEC:438438'. 
        // Vamos bloquear qualquer peça onde o chamado aplicado contenha 'TEC:' seguido de números pequenos (backups)
        // ou que não siga o padrão de OS (que costuma ser 6000... ou 5000...).
        
        if (chamado.startsWith('TEC:')) {
           const code = chamado.replace('TEC:', '').trim();
           // Se o código for pequeno (menos de 8 dígitos), geralmente é o ID do técnico/estoque e não uma OS.
