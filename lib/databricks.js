/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — Versão INFINITA (Paginada)
 */
...
/**
 * Executa query com suporte a paginação (traz TUDO)
 */
export async function runDatabricksQuery(sql, parameters = [], maxWaitMs = 300000) {
  ...
  // 3. Busca páginas adicionais se houver (PAGINAÇÃO)
  while (nextToken) {
...
/**
 * BUSCA EM MASSA: Busca peças de múltiplos técnicos de uma vez (FILTROS ORIGINAIS)
 */
export async function getAllTechniciansItems(technicianNames) {
  if (!technicianNames || technicianNames.length === 0) return [];

  // Lotes menores de técnicos para não sobrecarregar a query única
  const batchSize = 30; 
  let allResults = [];
