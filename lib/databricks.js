/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FIEL 2.0
 */
...
export async function runDatabricksQuery(sql, parameters = [], maxWaitMs = 300000) {
  ...
  // PAGINAÇÃO CORRETA: Percorre cada chunk pelo índice
...
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURADO: Exatamente seu SQL original
    const sql = `
      SELECT
