/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL ESTABILIZADA V2
 */
...
/**
 * BUSCA EM MASSA: Filtros originais + Filtro de Técnico Real (tecnico_tipo)
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // FILTRO DE TÉCNICO REAL:
    // Adicionado tecnico_tipo <> 'LABORATORIO' para remover remessas internas
    // que não pertencem ao estoque de campo do técnico.
    const sql = `
      SELECT
