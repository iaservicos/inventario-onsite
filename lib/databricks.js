/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO FINAL ESTABILIZADA
 */
...
/**
 * BUSCA EM MASSA: Restaura 100% dos filtros originais do usuário.
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURAÇÃO TOTAL:
    // Voltamos exatamente para o seu SQL original que trazia as peças do Marcio, Arnildo, etc.
    // Removidos os filtros extras de 'NÃO DEFINIDO' que estavam bloqueando o Marcio.
    const sql = `
      SELECT
