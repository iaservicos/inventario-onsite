...
/**
 * BUSCA EM MASSA: Filtros originais + Filtro de Encerramento
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURAÇÃO TOTAL V7:
    // 1. Filtros originais exatos.
    // 2. Filtro de Encerramento: Se o chamado já tem data de encerramento, a peça não é mais nova.
    const sql = `
      SELECT
      ...
      WHERE UPPER(TRIM(tecnico_nome)) IN (${placeholders})
        ...
        -- Regra de Ouro: Chamado encerrado = Peça não disponível
