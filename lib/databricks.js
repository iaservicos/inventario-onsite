/**
 * lib/databricks.js
 * Cliente de integração com Databricks SQL Warehouse — VERSÃO DE PRECISÃO
 */
...
export async function runDatabricksQuery(sql, parameters = [], maxWaitMs = 300000) {
  ...
  // 2. Pega as próximas páginas de forma sequencial (Chunk por Chunk)
...
/**
 * BUSCA EM MASSA: Com seus filtros originais restaurados 100%
 */
export async function getAllTechniciansItems(technicianNames) {
  ...
  for (let i = 0; i < technicianNames.length; i += batchSize) {
    ...
    // RESTAURADO: Seu SQL original exato
    const sql = `
      SELECT
