...
export async function getInventories({ from, to, technicianId, status, limit } = {}) {
  try {
    let query = db().from('inventories').select(`*, technicians(name, region)`).order('created_at', { ascending: false });
...
// Função de Alertas Simplificada para NÃO quebrar o Dashboard
export async function getAlerts() {
  try {
    const { data, error } = await db()
      .from('inventory_logs')
      .select('*') // Buscamos apenas os logs sem joins complexos primeiro
...
// Funções para o Power Automate (Dispatch)
...
// ... (mantenha tudo o que você já tem até a linha 205)

// ── ADICIONE ESTAS FUNÇÕES ABAIXO DA LINHA 205 ──────────────────────────
