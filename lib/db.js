...
export async function getInventories({ from, to, technicianId, status, limit } = {}) {
  try {
    // Mudança: Buscamos os dados de forma que, se o técnico não for encontrado, o inventário ainda apareça
    let query = db()
      .from('inventories')
      .select(`
        *,
    ...
    // Ajuste para o frontend não quebrar se o técnico vier como nulo
    return (data || []).map(inv => ({
      ...inv,
...
// Adicione isto ao final do seu arquivo lib/db.js

/**
 * Cria um agendamento e o inventário correspondente de uma só vez.
 * Isso garante que o Power Automate sempre encontre o inventory_id.
 */
export async function createScheduleWithInventory(supabase, { technician_id, scheduled_at, week_ref, notes, scheduled_subgroup }) {
  ...
  if (invError) {
    console.error("Erro ao criar inventário para agendamento:", invError);
    throw invError;
  }
  ...
  const { data: schedule, error: schError } = await supabase
    ...
    .insert({
      technician_id,
      inventory_id: inventory.id, // O VÍNCULO QUE ESTAVA FALTANDO!
      scheduled_at,
      week_ref,
      status: 'pending',
      notes,
  ...
  if (schError) {
    console.error("Erro ao criar agendamento:", schError);
    throw schError;
  }
