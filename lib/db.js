...
export async function getInventoryKPIs({ from, to, technicianId, technicianIds } = {}) {
  try {
    if (technicianIds !== null && technicianIds !== undefined && technicianIds.length === 0) {
      return { completed: 0, in_progress: 0, abandoned: 0, recount_pending: 0, divergences: 0, total: 0 };
    }
    let query = db().from('inventories').select('status, divergence_quantity');
    ...
    return {
      ...
      divergences: rows.filter((r) => (r.divergence_quantity ?? 0) > 0).length,
      total: rows.length,
    };
  } catch (e) {
