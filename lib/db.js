...
export async function createUser({ name, email, password_hash, role }) {
  const { data, error } = await db().from('users').insert({ name, email, password_hash, role }).select().single();
...
export async function getAllTechnicians() {
  const { data } = await db().from('technicians').select('*').order('name');
  return data || [];
}
    ...
    let query = db()
      ...
      .select(`
        *,
        ...
        inventory_schedules(scheduled_at, scheduled_subgroup)
      `)
...
export async function getAlerts({ from, to, technicianId, resolved } = {}) {
  try {
    let query = db()
      .from('alerts')
...
export async function getSubgroupForTechnician(supabase, technicianId, weekSubgroup) {
  const { data: allItems } = await supabase
    ...
    .select('item_subgroup')
    .eq('technician_id', technicianId)
    .eq('active', true)
    .not('item_subgroup', 'is', null);

  // Build map: lowercase key → original value (preserves case)
  const subgroupMap = new Map();
  for (const item of (allItems || [])) {
    const original = (item.item_subgroup || '').trim();
    if (!original) continue;
    const key = original.toLowerCase();
    if (!subgroupMap.has(key)) subgroupMap.set(key, original);
  }

  // Technician has no subgroup data at all
  if (subgroupMap.size === 0) return weekSubgroup;

  // Technician has items for the week's default subgroup — use it
  if (weekSubgroup) {
    const key = weekSubgroup.trim().toLowerCase();
    if (subgroupMap.has(key)) return subgroupMap.get(key);
  }

  // Check which subgroups this technician has already used
  ...
  const usedKeys = new Set(
    (history || []).map(h => (h.scheduled_subgroup || '').trim().toLowerCase()).filter(Boolean)
  );

  // Return first subgroup the technician hasn't used yet
  ...
  return weekSubgroup || [...subgroupMap.values()][0];
}
...
export async function getCountHistory({ from, to, technicianId, inventoryId } = {}) {
  try {
    let query = db()
      ...
      .select(`
        ...
        technicians!inventory_count_history_technician_id_fkey(id, name, region)
      `)
