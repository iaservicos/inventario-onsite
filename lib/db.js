...
export async function getConsolidatedTechnicianItems(supabase, technicianId, weekSubgroup) {
  ...
  if (weekSubgroup) {
    selectedItems = allItems.filter(item => 
      (item.item_subgroup || '').toLowerCase() === weekSubgroup.toLowerCase()
    );
  }

  // 3. SE NÃO HOUVER PEÇAS NO SUBGRUPO (ou se não houver subgrupo), PEGA TUDO
  // Isso garante que o agendamento nunca fique vazio se o técnico tiver peças
  if (selectedItems.length === 0) {
    selectedItems = allItems;
  }

  // 4. Consolidação por Código
