    ...
    <div style={{ padding: '1.5rem', maxWidth: '1400px' }}>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: 0 }}>
          {filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#a3a3a3' }}>Nenhum log encontrado</div>
          ) : (
            <div style={{ maxHeight: '600px', overflowY: 'auto' }}>
              {filtered.map((log, i) => (
                ...
                >
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: { info: '#a3a3a3', success: '#525252', warning: '#737373', error: '#171717' }[log.level] || '#a3a3a3', marginTop: '6px', flexShrink: 0 }} />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '2px' }}>
                      <span
                        style={{
                          fontSize: '0.7rem',
                          fontWeight: '600',
                          color: SOURCE_COLORS[log.source] || '#a3a3a3',
                          background: '#f5f5f5',
                          padding: '1px 6px',
                          borderRadius: '4px',
                      ...
                      <span style={{ fontSize: '0.7rem', color: '#a3a3a3', fontFamily: 'monospace' }}>{log.action}</span>
                    </div>
                    <div style={{ fontSize: '0.875rem', color: '#0a0a0a' }}>{log.message}</div>
                    {log.technicians?.name && (
                      <div style={{ fontSize: '0.75rem', color: '#a3a3a3', marginTop: '2px' }}>Técnico: {log.technicians.name}</div>
                    )}
                  </div>

                  <div style={{ fontSize: '0.7rem', color: '#a3a3a3', whiteSpace: 'nowrap', flexShrink: 0 }}>
...
function SummaryBadge({ label, value }) {
  return (
    <div style={{ padding: '0.4rem 0.75rem', background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
