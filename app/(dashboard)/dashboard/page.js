    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {lastUpdated && (
            <span style={{ fontSize: '0.7rem', color: '#8b949e', fontWeight: '600' }}>
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              borderRadius: '8px',
              padding: '1.5rem',
              boxShadow: '0 0 20px rgba(38, 208, 206, 0.05)',
            }}>
              ...
              <div style={{ height: '280px', width: '100%' }}>
                {pieData.length > 0 ? (
                  ...
                  <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8b949e', fontWeight: '600' }}>Sem dados para exibir</div>
                )}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginTop: '1rem', justifyContent: 'center' }}>
                {pieData.map((d) => (
                  <div key={d.name} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.7rem', fontWeight: '700', color: '#e6edf3' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxHeight: '300px', overflowY: 'auto' }}>
                {alerts.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '3rem', color: '#8b949e', fontWeight: '600' }}>Nenhum alerta ativo</div>
                ) : (
                  alerts.slice(0, 5).map((a) => (
                    <div key={a.id} style={{
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#8b949e', marginTop: '2px', fontWeight: '500' }}>{a.description}</div>
                    </div>
                  ))
                )}
              </div>
          ...
          <div style={{
            ...
            <div className="table-wrapper" style={{ border: 'none', boxShadow: 'none', borderRadius: '0' }}>
              <table>
                ...
                <tbody>
                  {recent.length === 0 ? (
                    <tr><td colSpan={9} style={{ textAlign: 'center', padding: '4rem', fontWeight: '700', color: '#8b949e' }}>Nenhum inventário recente</td></tr>
                  ) : (
                    recent.map((inv) => {
                      ...
                      return (
                        ...
                        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'rgba(38, 208, 206, 0.05)'}
                        onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                          <td style={{ fontWeight: '800', color: '#ffffff' }}>{inv.technicians?.name}</td>
                          <td style={{ fontWeight: '600', color: '#e6edf3' }}>{inv.technicians?.region || '—'}</td>
                          <td style={{ fontWeight: '600', color: '#e6edf3' }}>{inv.week_ref || '—'}</td>
                          <td>
                            {isGeral
                              ? <span className="badge badge-info" style={{ fontSize: '0.65rem' }}>GERAL</span>
                              : subgrupo
                                ? <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#e6edf3' }}>{subgrupo}</span>
                          ...
                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                              <div className="progress-bar" style={{ width: '80px', background: '#30363d', borderRadius: '10px', height: '6px', overflow: 'hidden' }}>
                          ...
                          <td style={{ fontWeight: '900', color: (inv.divergence_quantity ?? inv.divergence_count) > 0 ? '#f85149' : '#8b949e' }}>
                          ...
                          <td>
                            {scheduledAt ? (
                              <span style={{
                                ...
                                color: isOverdue ? '#f85149' : isPending ? '#e6edf3' : '#8b949e',
                          ...
                          <td>
                            {inv.started_at ? (
                              ...
                              <span style={{ fontSize: '0.7rem', fontWeight: '600', color: '#8b949e' }}>Aguardando</span>
                            )}
                          </td>
                        </tr>
                      );
  ...
  return (
    <div style={{
      background: highlight
        ? 'linear-gradient(135deg, rgba(38, 208, 206, 0.1) 0%, rgba(38, 208, 206, 0.02) 100%)'
        : '#161b22',
      border: highlight ? '1.5px solid #26d0ce' : '1px solid #30363d',
      borderRadius: '8px',
      padding: '1.5rem',
      boxShadow: highlight ? '0 0 20px rgba(38, 208, 206, 0.15)' : 'none',
    ...
    onMouseLeave={(e) => {
      ...
      <div style={{ fontSize: '0.7rem', fontWeight: '800', color: '#8b949e', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>{label}</div>
      <div style={{ fontSize: '2.25rem', fontWeight: '900', color: color, lineHeight: 1, marginBottom: '0.5rem' }}>{value}</div>
      {sub && <div style={{ fontSize: '0.75rem', color: '#8b949e', fontWeight: '600' }}>{sub}</div>}
    </div>
  );
}
