...
const STATUS_CFG = {
  ...
  em_validacao:         { label: 'Em Validação',      color: '#222', bg: 'var(--color-border-light)' },
  concluido:            { label: 'Concluído',          color: 'var(--color-bg-primary)', bg: '#222222' },
  com_divergencia:      { label: 'Com Divergência',    color: 'var(--color-bg-primary)', bg: 'var(--color-text-primary)' },
};
...
function ModalHeader({ title, subtitle, onClose }) {
  return (
    <div style={{ padding: '1.2rem 1.75rem', borderBottom: '1px solid var(--color-border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: 'var(--color-bg-tertiary)', flexShrink: 0 }}>
      ...
      <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: '1.1rem', cursor: 'pointer', color: 'var(--color-text-tertiary)', marginLeft: '1rem', lineHeight: 1 }}>✕</button>
    </div>
  );
}
      ...
      <div style={{ padding: '1.5rem 1.75rem', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        {selectedTech && (
          loadingInv ? (
            ...
            <div style={{ background: 'var(--color-bg-tertiary)', border: '1px solid var(--color-border-light)', borderRadius: '8px', padding: '1rem 1.25rem', fontSize: '0.82rem', color: 'var(--color-text-tertiary)', fontWeight: '600' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.82rem' }}>
                  ...
                  <tbody>
                    {toolInventory.map((item, i) => {
                      ...
                      return (
                        <tr key={item.tool_id} style={{ borderTop: i > 0 ? '1px solid var(--color-bg-tertiary)' : 'none' }}>
                          <td style={{ padding: '0.8rem 0.9rem' }}>
                            <div style={{ fontWeight: '700', color: 'var(--color-text-primary)' }}>{item.tool_name}</div>
                            {item.tool_notes && <div style={{ fontSize: '0.68rem', color: 'var(--color-text-tertiary)', marginTop: '0.15rem', fontStyle: 'italic' }}>⚠ {item.tool_notes}</div>}
                          </td>
                    ...
                    {v.validated_quantity > 0 ? (
                      <>
                        {novaBranchIdx !== i ? (
                          ...
                          }} style={{ ...inputSt, borderColor: !v.destination_branch ? '#999' : 'var(--color-border-light)', fontSize: '0.78rem' }}>
              ...
              <tr key={item.id} style={{ borderBottom: '1px solid var(--color-bg-tertiary)', background: i % 2 === 0 ? 'var(--color-bg-primary)' : 'var(--color-bg-tertiary)' }}>
                ...
                <td style={{ padding: '0.75rem 0.9rem' }}>
                  ...
                  {item.divergence_notes && (
                    <div style={{ fontSize: '0.65rem', color: 'var(--color-text-tertiary)', marginTop: '0.2rem', fontStyle: 'italic' }}>{item.divergence_notes}</div>
                  )}
                </td>
              </tr>
            ))}
    ...
    <div style={{ padding: '2rem', width: '100%', minHeight: '100vh', background: 'var(--color-bg-tertiary)', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ background: 'var(--color-bg-primary)', border: '1px solid var(--color-border-light)', borderRadius: '8px', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--color-text-tertiary)', fontWeight: '700' }}>Nenhuma devolução registrada.</div>
        ) : (
