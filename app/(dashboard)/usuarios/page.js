      ...
      {showForm && (
        <div className="card" style={{ marginBottom: '1.5rem' }}>
          ...
          <form onSubmit={createUser} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '500', color: '#52525b', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Nome</label>
              <input className="input" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nome completo" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '500', color: '#52525b', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>E-mail</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '500', color: '#52525b', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Senha</label>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '500', color: '#52525b', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Perfil</label>
      ...
      <div className="card">
        <div className="table-wrapper">
          <table>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', color: '#52525b', padding: '2rem' }}>Nenhum usuário encontrado</td></tr>
              ) : users.map((u) => (
                <tr key={u.id}>
                  <td style={{ fontWeight: '500', color: '#f4f4f5' }}>{u.name}</td>
                  ...
                  <td>
                    <span style={{ fontSize: '0.75rem', color: u.active ? '#f4f4f5' : '#52525b' }}>
                      {u.active ? 'Ativo' : 'Inativo'}
                    </span>
                  </td>
                  <td style={{ color: '#71717a', fontSize: '0.8rem' }}>{formatDate(u.created_at)}</td>
                  <td>
